...
if(isset($_POST["q"]) && $_POST["q"] == '1') {
    ...
    if ($info[5] == 'PEO'){
        $sql3 ="UPDATE CON_CLARITY_SOLIST SET CONTRATOR=:CONTRACTOR,STATUS='1',UPDATE_USER='$user',STATUS_DATE=sysdate
                WHERE SO_NUM = :SO_NUM AND STATUS='0' and EX_NO = :EX_NO";
        $stmt3 = $oraconn->prepare($sql3);
        $stmt3->execute(['CONTRACTOR' => $info[1], 'SO_NUM' => $sod,'EX_NO' => $info[3]]);
    }else{
        $sql3 ="UPDATE CON_CLARITY_SOLIST SET CONTRATOR=:CONTRACTOR,STATUS='1',UPDATE_USER='$user',STATUS_DATE=sysdate
                WHERE VOICENUMBER = :VOICENUMBER AND STATUS='0' and EX_NO = :EX_NO";
        $stmt3 = $oraconn->prepare($sql3);
        $stmt3->execute(['CONTRACTOR' => $info[1], 'VOICENUMBER' => $info[0],'EX_NO' => $info[3]]);
    }
...
if(isset($_POST["q"]) && $_POST["q"]== 'addteam' ) {
    ...
    $sql= "INSERT INTO CONTRACTOR_TEAMS (AREA, CONTRACTOR, TEAM_DATE,S_TYPE, TEAM_COUNT, EN_USER, TEAM_FLAG) 
...
if(isset($_POST["q"]) && $_POST["q"]== 'addmobuser' ) {
    ...
    $sql= "INSERT INTO TECHS_CON_USER (CON_USER, CON_USER_CONTACT, CON_CONTRACTOR, CON_TEAM_ID) 
...
if(isset($_POST["q"]) && $_POST["q"]== 'updatemetFtth' ) {
    ...
    $sql= "UPDATE  CONTRACTOR_FTTH_MET SET P0 = :P0 where SOID = :SOID and MET_ID= :MET_ID and UNIT_DESIG=:UNIT_DESIG";
...
if(isset($_POST["q"]) && $_POST["q"]== 'deleteMet' ) {
    ...
    $sql= "DELETE FROM  CONTRACTOR_FTTH_MET where SOID = :SOID and MET_ID= :MET_ID and UNIT_DESIG=:UNIT_DESIG";
...
if(isset($_POST["q"]) && $_POST["q"]== 'addMeterial' ) {
    ...
    $sql= "INSERT INTO CONTRACTOR_FTTH_MET(SOID, VOICENO,UNIT_DESIG,P0,P1,SN,MET_ID)
...
if(isset($_POST["q"]) && $_POST["q"]== 'returnSod' ) {
    ...
    $sql= "update  CONTRACTOR_WORK_ORDERS set CON_STATUS_DATE = sysdate, CON_STATUS = 'RETURN_SLT' where CON_SERO_ID = :CON_SERO_ID and CON_WORO_TASK_NAME= :CON_WORO_TASK_NAME";
    $oraconn = OracleConnection();
    $stmt = $oraconn->prepare($sql);
    $stmt->execute(['CON_SERO_ID' => $info[0],'CON_WORO_TASK_NAME' =>$info[1]]);


    $sql2= "update  CONTRACTOR_NEW_CON set CON_SO_COM_DATE = sysdate, CON_SO_STATUS = 'RETURN_SLT'  where CON_SO_ID = :CON_SO_ID and CON_TASK=:CON_TASK";
    $stmt2 = $oraconn->prepare($sql2);
    $stmt2->execute(['CON_SO_ID' => $info[0],'CON_TASK'=>$info[1]]);

    $sql3= "delete from  SERIAL_NO_SO  where SO_ID = :SO_ID";
    $stmt3 = $oraconn->prepare($sql3);
    $stmt3->execute(['SO_ID' => $info[0]]);
...
if(isset($_POST["q"]) && $_POST["q"]== 'returnSodsave') {

    $info=$_POST["info"];

    if($info[3] == 'CONSTRUCT_OSP'){
        $sql ="UPDATE TECHS_RET_REASONS SET RETURNED_REASON=:RETURNED_REASON, RETURNED_COMMENT=:RETURNED_COMMENT,
           RETURNED_DATE= sysdate WHERE  SOID = :SOID AND TASK_NAME = 'RETURNOSP' ";
    }
    if($info[3] == 'RECONSTRUCT_OSP'){
        $sql ="UPDATE TECHS_RET_REASONS SET RETURNED_REASON=:RETURNED_REASON, RETURNED_COMMENT=:RETURNED_COMMENT,
           RETURNED_DATE= sysdate WHERE  SOID = :SOID AND TASK_NAME = 'RETURNSECONDOSP'  ";
    }


    $oraconn = OracleConnection();
    $stmt = $oraconn->prepare($sql);
    $stmt->execute(['RETURNED_REASON' => $info[1],'RETURNED_COMMENT' => $info[2],'SOID' => $info[0]]);
    ...
    $msg = 'Return Reason updated '.$info[1];
    log_all($user,$info[0],$msg);
...
if(isset($_POST["q"]) && $_POST["q"]== 'patStatusOpmc') {

    $info=$_POST["info"];

    if($info[1] == 'CONFIRM'  ) {
        $sql= "update  CONTRACTOR_WORK_ORDERS set CON_STATUS_DATE = sysdate, CON_STATUS = 'PAT_OPMC_PASSED' where CON_SERO_ID = :CON_SERO_ID";
        $oraconn = OracleConnection();
        $stmt = $oraconn->prepare($sql);
        $stmt->execute(['CON_SERO_ID' => $info[0]]);

        $sql4 = "insert into TECHS_OPMCPAT_DETAILS values ('$info[0]','$user',sysdate,'PAT_OPMC_PASSED')";
        $oraconn4 = OracleConnection();
        $stmt4 = $oraconn4->prepare($sql4);
        $stmt4->execute();
    }
    if($info[1] == 'REJECT'  ) {
        $sql= "update  CONTRACTOR_WORK_ORDERS set CON_STATUS_DATE = sysdate, CON_STATUS = 'PAT_OPMC_REJECTED' where CON_SERO_ID = :CON_SERO_ID";
        $oraconn = OracleConnection();
        $stmt = $oraconn->prepare($sql);
        $stmt->execute(['CON_SERO_ID' => $info[0]]);
        ...
        $sql4 = "insert into TECHS_OPMCPAT_DETAILS values ('$info[0]','$user',sysdate,'PAT_OPMC_REJECTED')";
        $oraconn4 = OracleConnection();
        $stmt4 = $oraconn4->prepare($sql4);
        $stmt4->execute();
        ...
        $sql3 = "insert into TECHS_PAT_COMMENT values ('$info[0]','$info[2]',sysdate)";
        $oraconn3 = OracleConnection();
        $stmt3 = $oraconn3->prepare($sql3);
        $stmt3->execute();
    ...
    $msg = 'PAT OPMC Results Completed '.$info[1].' by '.$user;
    log_all($user,$info[0],$msg);
...
if(isset($_POST["q"]) && $_POST["q"]== 'returnStatus' ) {

    $info=$_POST["info"];

    $sql= "update  CONTRACTOR_WORK_ORDERS set CON_STATUS_DATE = sysdate, CON_STATUS = 'RETURN_CLOSED' where CON_SERO_ID = :CON_SERO_ID and CON_WORO_TASK_NAME= :CON_WORO_TASK_NAME";
    $oraconn = OracleConnection();
    $stmt = $oraconn->prepare($sql);
    $stmt->execute(['CON_SERO_ID' => $info[0],'CON_WORO_TASK_NAME' => $info[0]]);

    $sql3= "update  CON_CLARITY_SOLIST set STATUS_DATE = sysdate, STATUS = '30'  where SO_NUM = :SO_NUM";
    $stmt3 = $oraconn->prepare($sql3);
    $stmt3->execute(['SO_NUM' => $info[0]]);

    $sql2= "update  CONTRACTOR_NEW_CON set CON_SO_COM_DATE = sysdate, CON_SO_STATUS = 'RETURN_CLOSED'  where CON_SO_ID = :CON_SO_ID and CON_TASK=:CON_TASK";
    $stmt2 = $oraconn->prepare($sql2);
    $stmt2->execute(['CON_SO_ID' => $info[0],'CON_TASK' => $info[1]]);

//    $sql4= "update  SERIAL_NO_SO set STATUS = '20'  where SO_ID = :SO_ID";
//    $stmt4 = $oraconn->prepare($sql4);
//    $stmt4->execute(['SO_ID' => $info[0]]);
    ...
    $msg = 'Service Order Returned to OSS'.$info[0];
    log_all($user,$info[0],$msg);

}


if(isset($_POST["q"]) && $_POST["q"]== 'returnSameCon' ) {

    $info=$_POST["info"];

    $sql= "update  CONTRACTOR_WORK_ORDERS set CON_STATUS_DATE = sysdate, CON_STATUS = 'INPROGRESS' where CON_SERO_ID = :CON_SERO_ID and CON_WORO_TASK_NAME= :CON_WORO_TASK_NAME";
    $oraconn = OracleConnection();
    $stmt = $oraconn->prepare($sql);
    $stmt->execute(['CON_SERO_ID' => $info[0],'CON_WORO_TASK_NAME' => $info[1]]);

    $sql2= "update  CONTRACTOR_NEW_CON set CON_SO_COM_DATE = sysdate, CON_SO_STATUS = 'INPROGRESS'  where CON_SO_ID = :CON_SO_ID and CON_TASK=:CON_TASK";
    $stmt2 = $oraconn->prepare($sql2);
    $stmt2->execute(['CON_SO_ID' => $info[0],'CON_TASK' => $info[1]]);

    $sql3= "update  CON_CLARITY_SOLIST set STATUS_DATE = sysdate, STATUS = '1'  where SO_NUM = :SO_NUM";
    $stmt3 = $oraconn->prepare($sql3);
    $stmt3->execute(['SO_NUM' => $info[0]]);

    $sql4= "delete from  SERIAL_NO_SO  where SO_ID = :SO_ID";
    $stmt4 = $oraconn->prepare($sql4);
    $stmt4->execute(['SO_ID' => $info[0]]);

    $sql5= "delete from  TECHS_RET_REASONS  where SOID = :SO_ID";
    $stmt5 = $oraconn->prepare($sql5);
    $stmt5->execute(['SO_ID' => $info[0]]);
    ...
    $msg = 'Service Order Re Assigned to same Contractor ';
    log_all($user,$info[0],$msg);

}


if(isset($_POST["q"]) && $_POST["q"]== 'returnChangeCon' ) {

    $info=$_POST["info"];

    $sql= "update  CONTRACTOR_WORK_ORDERS set CON_STATUS_DATE = sysdate, CON_STATUS = 'ASSIGNED',CON_NAME=:CON_NAME where CON_SERO_ID = :CON_SERO_ID and CON_WORO_TASK_NAME= :CON_WORO_TASK_NAME";
    $oraconn = OracleConnection();
    $stmt = $oraconn->prepare($sql);
    $stmt->execute(['CON_SERO_ID' => $info[0],'CON_NAME' => $info[1],'CON_WORO_TASK_NAME' => $info[3]]);

    $sql2= "update  CONTRACTOR_NEW_CON set CON_SO_COM_DATE = sysdate, CON_SO_STATUS = 'ASSIGNED',CON_CONTRACTOR=:CON_CONTRACTOR  where CON_SO_ID = :CON_SO_ID and CON_TASK=:CON_TASK";
    $stmt2 = $oraconn->prepare($sql2);
    $stmt2->execute(['CON_SO_ID' => $info[0],'CON_CONTRACTOR' => $info[1],'CON_TASK' => $info[3]]);

    $sql3= "update  CON_CLARITY_SOLIST set STATUS_DATE = sysdate, STATUS = '1',CONTRATOR=:CONTRATOR  where SO_NUM = :SO_NUM";
    $stmt3 = $oraconn->prepare($sql3);
    $stmt3->execute(['SO_NUM' => $info[0],'CONTRATOR' => $info[1]]);
    ...
    $msg = 'Service Order Re Assigned to new Contractor '.$info[1];
    log_all($user,$info[0],$msg);
...
if(isset($_POST["q"]) && $_POST["q"]== 'addAcptCmt') {
    ...
    $sql= "DELETE FROM SERIAL_NO_IMAGES WHERE SOID= :SOID AND IMAGEID = :IMGID AND IMAGE_NAME IS NULL";
    $stmt = $oraconn->prepare($sql);
    $stmt->execute(['SOID' => $info[0],'IMGID' => $info[1]]);

    $sql= "UPDATE SERIAL_NO_IMAGES SET IMAGE_COMMENT = :IMG_CMT, IMAGE_COMUSER = :IMG_CMT_USER,IMAGE_COM_DATE = SYSDATE,STATUS = '10',STATUS_DATE=SYSDATE WHERE SOID = :SO_ID AND IMAGEID = :IMG_ID AND IMAGE_NAME = :IMG_NAME AND IMAGE_NAME IS NOT NULL";
    $stmt = $oraconn->prepare($sql);
...
if(isset($_POST["q"]) && $_POST["q"]== 'addRjCmt') {
    ...
    $sql= "INSERT INTO SERIAL_NO_IMAGES (SOID,IMAGEID,IMAGE_DISNAME,IMAGE_STAGE) VALUES(:SOID,:IMGID,:IMGNAME,:IMG_STG)";
    $stmt = $oraconn->prepare($sql);
    $stmt->execute(['SOID' => $info[0],'IMGID' => $info[1],'IMGNAME' => $info[4],'IMG_STG' => $imgStg]);

    $sql= "UPDATE SERIAL_NO_IMAGES SET IMAGE_COMMENT = :IMG_CMT, IMAGE_COMUSER = :IMG_CMT_USER,IMAGE_COM_DATE = SYSDATE,STATUS = '5',STATUS_DATE=SYSDATE WHERE SOID = :SO_ID AND IMAGEID = :IMG_ID AND IMAGE_NAME = :IMG_NAME AND IMAGE_NAME IS NOT NULL";
    $stmt = $oraconn->prepare($sql);
    $stmt->execute(['IMG_CMT' => $info[2],'IMG_CMT_USER' => $user,'SO_ID' => $info[0],'IMG_ID' => $info[1],'IMG_NAME' => $info[3]]);
...
if(isset($_POST["q"]) && $_POST["q"]== 'addGenCmt' ) {
    ...
    $sql= "INSERT INTO PAT_COMMENT(SO_ID, PAT_GEN_CMT,CMT_USER,CMT_DATE) VALUES ('$info[0]','$info[1]','$user',SYSDATE)";
...
if(isset($_POST["q"]) && $_POST["q"]== 'loadServices') {

        $info=$_POST["info"];

        $sql = "select distinct * from SOD_BEARER_MAP where BEARER_SOD = :SO_ID order by SOD";
        $oraconn = OracleConnection();
        $stmt = $oraconn->prepare($sql);
        $stmt->execute(['SO_ID' => $info[0]]);
        $data = $stmt->fetchAll();
...
if(isset($_POST["q"]) && $_POST["q"]== 'addImgUpSt' ) {
    ...
    $sql= "UPDATE  CONTRACTOR_NEW_CON SET FTTH_WIFI = :OFLN_ST where CON_SO_ID  = :SO_ID";

    $oraconn = OracleConnection();
    $stmt = $oraconn->prepare($sql);
    $stmt->execute(['SO_ID' => $info[0],'OFLN_ST' => $info[2]]);

    if($info[2] == 1){

        $msg = 'Selected To Offline Proceed '.$user;
        log_all($user,$info[0],$msg);
...
if(isset($_POST["q"]) && $_POST["q"]== 'upSodStatus') {

    $info=$_POST["info"];

    $sql= "UPDATE CONTRACTOR_NEW_CON set CON_SO_STATUS = 'OSP_CLOSED', CON_SO_STATUS_DATE = sysdate where CON_SO_ID = :SO_ID";

    $oraconn = OracleConnection();
    $stmt = $oraconn->prepare($sql);
    $stmt->execute(['SO_ID' => $info[0]]);

    $sql2= "UPDATE CONTRACTOR_WORK_ORDERS set CON_STATUS = 'OSP_CLOSED', CON_STATUS_DATE = sysdate where CON_SERO_ID = :SO_ID";

    $stmt2 = $oraconn->prepare($sql2);

    if($stmt2->execute(['SO_ID' => $info[0]])){
        $result = 'success';
    }else{
        $result = 'failed';
    }
...
if(isset($_POST["q"]) && $_POST["q"]== 'getExPolCnt') {

    $info=$_POST["info"];

    $sql ="SELECT POLE_COUNT FROM CONTRACTOR_NEW_CON WHERE CON_SO_ID = '".$info[0]."'";
    $oraconn = OracleConnection();
    $stmt = $oraconn->prepare($sql);
    $stmt->execute();
    $result = $stmt->fetch();
...
if(isset($_POST["q"]) && $_POST["q"]== 'delExPolImg' ) {

    $soid=$_POST["soid"];
    $polimgN = $_POST["polimgN"];

    $sql= "DELETE FROM  SERIAL_NO_IMAGES where SOID = :SOID AND IMAGE_DISNAME like '%Pole%' AND IMAGE_DISNAME LIKE '".$polimgN."'";
...
if(isset($_POST["q"]) && $_POST["q"]== 'upSrlNSo') {

    $info=$_POST["info"];

    $sql= "UPDATE  SERIAL_NO_SO SET STATUS = '100' where SO_ID  = :SO_ID AND STATUS < '10' ";
