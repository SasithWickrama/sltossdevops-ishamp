...
class DbOperations
{
    ...
    public function getCont(){
        $sql ="select distinct CON_MGT_CATOGARY from TECHS_MGT_LOGIN 
               where CON_MGT_CATOGARY <> :CON_MGT_CATOGARY order by CON_MGT_CATOGARY";

        $stmt = $this->con->prepare($sql);
        $stmt->execute(['CON_MGT_CATOGARY' => 'SLT']);
        $result = $stmt->fetchAll();
        if($result){
            return $result;
        }else{
    ...
    function getMobTeam($con)
    {
        $sql ="select USERNAME from APP_LOGIN where CONTRACTOR  = :CONTRACTOR";
        $stmt = $this->con->prepare($sql);
    ...
    function getLog($sod)
    {
        $sql = "select CON_USER,to_char(LOG_DATE, 'mm/dd/yyyy hh:mi:ss AM') LDATE,SO_ID,MSG 
        from CONTRACTOR_LOG where SO_ID= :SOID order by LDATE DESC ";
        $stmt = $this->con->prepare($sql);
        $stmt->execute(['SOID' => $sod]);
        $result = $stmt->fetchAll();
        if($result){
    ...
    function getNewSodOsp($a,$b)
    {
        ...
        and C.CON_STATUS IN ('ASSIGNED', 'INPROGRESS','OSP_CLOSED','INSTALL_CLOSED','COMPLETED')";
    ...
    function getOthers($con)
    {
        $sql = "select UNIT_DESIG from CONTRACTOR_UD where STYPE = 'FTTH' and  NEW= :NEW";
    ...
    {
        $sql = "select distinct CON_MGT_UNAME from TECHS_MGT_LOGIN where CON_MGT_MOBILE = :CON_MGT_MOBILE ";
        $stmt = $this->con->prepare($sql);
        $stmt->execute(['CON_MGT_MOBILE' => $mobile]);
        $result = $stmt->fetch();
        if($result){
            return $result;
        }
