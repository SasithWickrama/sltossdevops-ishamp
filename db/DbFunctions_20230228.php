...
if(isset($_POST["q"]) && $_POST["q"]== 'returnStatus' ) {
    ...
    $sql= "update  CONTRACTOR_WORK_ORDERS set CON_STATUS_DATE = sysdate, CON_STATUS = 'RETURN_CLOSED' where CON_SERO_ID = :CON_SERO_ID";
    $oraconn = OracleConnection();
    $stmt = $oraconn->prepare($sql);
    $stmt->execute(['CON_SERO_ID' => $info[0]]);
    ...
    $sql2= "update  CONTRACTOR_NEW_CON set CON_SO_COM_DATE = sysdate, CON_SO_STATUS = 'RETURN_CLOSED'  where CON_SO_ID = :CON_SO_ID";
    $stmt2 = $oraconn->prepare($sql2);
    $stmt2->execute(['CON_SO_ID' => $info[0]]);

    $sql4= "update  SERIAL_NO_SO set STATUS = '50'  where SO_ID = :SO_ID";
    $stmt4 = $oraconn->prepare($sql3);
    $stmt4->execute(['SO_ID' => $info[0]]);
    ...
    $msg = 'Service Order Returned '.$info[0];
    log_all($user,$info[0],$msg);
